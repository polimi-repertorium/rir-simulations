classdef RT60functionsContainer
    methods
        function T60 = Estimate_RT60(~,IR,Fs,EDC_region,plot_ok)
        %--------------------------------------------------------------------------
        ...
        function RT60 = sabine_formula(~, room_dim, beta_coeff)
           A = ((room_dim(1) * room_dim(3) * 2) + (room_dim(2) * room_dim(3)*2) + (room_dim(1)*room_dim(2)*2)) * (1-beta_coeff) ;
           display(A)
           RT60 = 0.049*((room_dim(1) * room_dim(2) * room_dim(3))/A);
        end
    end
end
